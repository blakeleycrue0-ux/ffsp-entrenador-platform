  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <nav className="hidden items-center gap-7 md:flex">
            {[
              ...
              <a key={href} href={href} className="text-[14px] text-navy-600 transition-colors hover:text-navy-900">
                {label}
              </a>
            ))}
          </nav>
          <div className="flex items-center gap-2">
            <a href="#plataforma" className="hidden text-[14px] font-medium text-navy-600 transition-colors hover:text-navy-900 sm:block">
      ...
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-[520px] bg-gradient-to-b from-navy-50/70 to-transparent" />
        <div className="relative mx-auto max-w-6xl px-5 pb-16 pt-16 sm:pt-24">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-7 flex justify-center">
              <Crest size={92} />
            </div>
            <Tag tone="solid" className="mb-5">
              FFSP · Santa Ponsa CF
            </Tag>
            <h1 className="text-[34px] font-semibold leading-[1.1] tracking-[-0.02em] text-navy-900 sm:text-[52px]">
              El centro de operaciones
              <br className="hidden sm:block" /> de la entrenadora.
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-[16px] leading-relaxed text-muted sm:text-[17.5px]">
              Planifica, gestiona, comunica y mejora desde un único lugar. Menos gestión, más tiempo para entrenar.
            </p>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              <Link
                to="/entrar"
                className="inline-flex h-12 items-center gap-2 rounded-xl bg-navy-900 px-7 text-[15px] font-medium text-white shadow-raised transition-all hover:-translate-y-0.5 hover:bg-navy-900"
              >
                Entrar <ArrowRight size={17} />
              </Link>
              <a
                href="#plataforma"
                className="inline-flex h-12 items-center rounded-xl border border-line bg-white px-6 text-[15px] font-medium text-navy-700 transition-colors hover:border-navy-300 hover:text-navy-900"
              >
                Conocer la plataforma
              </a>
            </div>
            <p className="mt-5 text-[13px] text-navy-400">
      ...
      {/* Problema → solución */}
      <section className="border-y border-line/70 bg-navy-50/50 py-16">
            ...
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {['WhatsApp', 'Calendario', 'Notas', 'Excel', 'PDFs', 'Grupos', 'Documentos'].map((t) => (
                  <span key={t} className="rounded-lg border border-line bg-white px-2.5 py-1.5 text-[12.5px] text-navy-400 line-through">
                    {t}
                  </span>
                ))}
              </div>
            ...
            {MODULES.map((m) => {
              ...
              return (
                <div key={m.title} className="card card-hover p-5">
                  <span className="grid h-11 w-11 place-items-center rounded-xl bg-navy-50 text-navy-900">
                    <Icon size={20} />
                  </span>
                  <h3 className="mt-4 text-[15.5px] font-semibold">{m.title}</h3>
                  <p className="mt-2 text-[13.5px] leading-relaxed text-muted">{m.text}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* IA */}
      <section id="ia" className="border-y border-line/70 bg-navy-50/50 py-20">
        <div className="mx-auto grid max-w-6xl gap-12 px-5 lg:grid-cols-2 lg:items-center">
          <div>
            <Tag tone="solid">FFSP Assistant</Tag>
            ...
            <ul className="mt-6 space-y-3">
              {[
                ...
                <li key={q} className="flex items-start gap-3 rounded-xl border border-line bg-white px-4 py-3">
                  <Sparkles size={16} className="mt-0.5 shrink-0 text-navy-700" />
                  <span className="text-[14px] text-navy-700">{q}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-6 space-y-3">
              {[
                ...
                <div key={t} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[14.5px] font-medium text-navy-800">{t}</p>
                    <p className="mt-0.5 text-[13.5px] leading-relaxed text-muted">{d}</p>
                  </div>
                </div>
              ))}
            </div>
            <p className="mt-6 rounded-xl border border-warn/30 bg-warn/5 px-4 py-3 text-[13px] leading-relaxed text-[#8A5A10]">
      ...
      {/* Cierre */}
      <section className="border-t border-line/70 bg-navy-50/40 py-20">
        <div className="mx-auto max-w-3xl px-5 text-center">
          ...
          <Link
            to="/entrar"
            className="mt-8 inline-flex h-12 items-center gap-2 rounded-xl bg-navy-900 px-7 text-[15px] font-medium text-white shadow-raised transition-all hover:-translate-y-0.5 hover:bg-navy-900"
          >
            Entrar <ArrowRight size={17} />
          </Link>
        </div>
...
function DashboardPreview() {
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-pop">
      <div className="flex items-center gap-2 border-b border-line/80 bg-navy-50/70 px-4 py-2.5">
        <span className="flex gap-1.5">
          {[0, 1, 2].map((i) => (
            <span key={i} className="h-2.5 w-2.5 rounded-full bg-line" />
          ))}
        </span>
        <span className="ml-3 rounded-md bg-white px-2.5 py-1 text-[11px] text-navy-400 ring-1 ring-line">
          Tu día, en una pantalla
        </span>
      </div>

      <div className="grid grid-cols-[190px_1fr] max-sm:grid-cols-1">
        <div className="border-r border-line/70 p-3 max-sm:hidden">
          <Wordmark size="sm" showSubtitle={false} />
          <div className="mt-4 rounded-lg bg-navy-900 px-3 py-2 text-[12px] font-medium text-white">+ Crear</div>
            ...
            {[
              ...
              <div
                ...
                className={cn(
                  ...
                  active ? 'bg-navy-50 font-medium text-navy-900' : 'text-muted',
                )}
              >
                {label as string}
              </div>
        ...
        <div className="p-4 sm:p-5">
          <p className="text-[17px] font-semibold text-navy-900">Hola 👋</p>
          <p className="mt-0.5 text-[12px] text-muted">Esto es lo que tienes preparado para hoy</p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-xl border border-line p-3.5">
                <span className="text-[9.5px] font-semibold uppercase tracking-wider text-navy-400">{label}</span>
                <p className="mt-2 text-[13.5px] font-semibold leading-tight text-navy-900">{title}</p>
                <p className="mt-1 text-[11.5px] leading-relaxed text-muted">{hint}</p>
                <div className="mt-3 flex gap-1">
                  {[10, 15, 20, 25].map((w, i) => (
                    <div key={i} className="h-1.5 rounded-full bg-navy-200" style={{ flex: w }} />
                  ))}
                </div>
              </div>
            ))}
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-pop">
      <div className="flex items-center gap-3 border-b border-line/80 px-5 py-3.5">
        <span className="grid h-8 w-8 place-items-center rounded-xl bg-navy-900">
          <Sparkles size={15} className="text-white" />
        </span>
        <div>
          <p className="text-[13.5px] font-semibold text-navy-900">FFSP Assistant</p>
          <p className="text-[11px] text-navy-400">Con el contexto de tu equipo</p>
        </div>
      </div>
      <div className="space-y-4 p-5">
        <div className="flex justify-end">
          <p className="max-w-[85%] rounded-2xl rounded-br-md bg-navy-900 px-3.5 py-2.5 text-[12.5px] leading-relaxed text-white">
            Prepárame un entrenamiento de 90 minutos centrado en presión tras pérdida.
          </p>
        </div>
        <div className="flex gap-2.5">
          <span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-navy-50">
            <Sparkles size={13} className="text-navy-800" />
          </span>
          <div className="flex-1 space-y-2.5">
            <p className="rounded-2xl rounded-tl-md bg-navy-50 px-3.5 py-2.5 text-[12.5px] leading-relaxed text-navy-700">
              Monta la sesión con los ejercicios de tu biblioteca, reparte la carga y te la deja lista para revisar.
            </p>
            <div className="overflow-hidden rounded-xl border border-line">
              <div className="flex items-center justify-between border-b border-navy-100 bg-navy-50/40 px-3.5 py-2">
                <p className="text-[12px] font-semibold text-navy-900">Presión tras pérdida — 90′</p>
                <span className="rounded-full bg-navy-100 px-2 py-0.5 text-[10px] font-medium text-navy-900">1h 30′</span>
              </div>
              {[
                ...
                <div key={n} className="flex items-center gap-2.5 border-b border-navy-100 px-3.5 py-2 last:border-0">
                  <span className="grid h-5 w-5 place-items-center rounded bg-navy-100 text-[9.5px] font-semibold text-muted">{n}</span>
                  <span className="flex-1 truncate text-[12px] text-navy-700">{t}</span>
                  <span className="text-[11px] font-medium text-muted">{d}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <span className="rounded-lg bg-navy-900 px-3 py-1.5 text-[11.5px] font-medium text-white">Guardar entrenamiento</span>
              <span className="rounded-lg border border-line px-3 py-1.5 text-[11.5px] font-medium text-navy-600">Editar</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-pop">
      <div className="flex items-center justify-between border-b border-line/80 px-5 py-3.5">
        <p className="text-[13.5px] font-semibold text-navy-900">Vista previa del mensaje</p>
        <span className="rounded-full bg-warn/10 px-2.5 py-1 text-[10.5px] font-medium text-[#9A6412] ring-1 ring-inset ring-warn/25">
          No enviado
        </span>
      </div>
      <div className="bg-[#ECE5DD] p-5">
        <div className="rounded-xl rounded-tl-sm bg-white p-3.5 shadow-sm">
          <p className="whitespace-pre-line text-[12.5px] leading-relaxed text-navy-800">
            {`*CONVOCATORIA*
...
✅ Voy   ❌ No puedo   ❓ Aún no lo sé`}
          ...
          <p className="mt-2 text-right text-[10px] text-navy-400">vista previa · no enviado</p>
        </div>
      </div>
      <div className="flex items-center gap-2 border-t border-line/80 px-5 py-3.5">
        <Button size="sm" variant="secondary">Editar</Button>
        <Button size="sm" variant="secondary" icon={<Send size={15} />}>
          Enviar por WhatsApp
        </Button>
      </div>
    </div>
